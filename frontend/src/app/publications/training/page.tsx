...
export default function Page() {
  ...
  useEffect(() => {
    fetch('/api/publications?type=TRAINING&limit=50')
      .then(r => r.json()).then(d => { setItems(d.publications || []); setLoading(false); }).catch(() => setLoading(false));
  }, []);
  return (
    <PageLayout titleKey="publications.training.title" subtitleKey="publications.training.description" backHref="/publications" backLabelKey="publications.label">
